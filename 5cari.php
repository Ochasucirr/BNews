...
<body>
    ...
    <div class="all container">
        <div class="articles-card">
            ...
            <?php
            // Pencarian
            $kataKunci = isset($_GET['kata_kunci']) ? $_GET['kata_kunci'] : '';
            $hasilPencarian = cariArtikel($kataKunci);

            // Inisialisasi variabel untuk pesan error
            $error_message = '';
            ?>

            <!-- Jika ada hasil pencarian, urutkan dan ambil 5 artikel teratas -->
            <?php if (empty($hasilPencarian)): ?>
                <div class="alert">
                    <p>Sorry, no articles found with keywords '<?= $kataKunci ?>'</p>
                </div>
            <?php else: ?>
                <!-- Tampilkan hasil pencarian -->
                <?php $top_5_articles = array_slice($hasilPencarian, 0, 5); ?>
